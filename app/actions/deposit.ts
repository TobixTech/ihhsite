...
export async function approveDeposit(reference: string) {
  ...
  // If already SUCCESS, still run the wallet upsert — it may never have been credited
  const alreadySuccess = dep.status === "success"
  ...
  // Only insert a transaction record if not already approved to avoid duplicates
  if (!alreadySuccess) {
